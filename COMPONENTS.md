...
- Client 半（`src/client/index.ts`）：`inject = ['slots']`，在
  ...
  - 另注册 `widgets.config`，id **`rainbow-flow`**，order **0** →
    ...
    （`RainbowFlowSettings` + `DEFAULT_SETTINGS` / `DEFAULT_TOOL_COLORS` +
    `loadSettings`/`saveSettings`/`subscribeSettings`/`getSettings`，uSES store，
    `toolColors` 为 `Record<ToolCategory,string>`，持久化 `dsh.rnglow.settings`，
    含旧 `wisps` 键的持久化数据被忽略），已挂载的光环实时生效（透明度缩放呼吸
    幅度、速度缩放 `rateToDuration` 映射、冷色调开关控制暖层→冷层交叉淡化）；
    命令颜色经 `mountToolAccent()` 写成 `<html>` 内联 `--rf-tool-*` 覆盖样式表
    `:root` 默认，已上色的命令卡实时变色。
  ...
  - **会话命令卡按类型上色**（`src/client/toolAccent.ts` + `classify.ts` +
    ...
    **读/写/编辑类工具的文件路径**是 `_fileLink` 可点击链接（不是 `_summary`），
    ...
    `#c084fc`）并同款上色。全套 12 色**一色一家族、明显可区分**（暖色系 绿/黄/橙/棕，
     冷色系 蓝→青→靛→紫→淡紫 逐一错开，红色专属 ask，仅 other 为灰）；**每类颜色可按用户配置自定义**（经 `settings.ts` 的 `toolColors` → 本模块写成 `<html>` 内联 `--rf-tool-*`，覆盖样式表 `:root` 默认并实时生效，配置面板「命令文字颜色」编辑）；**最新行动（命令/思考）的标题/摘要被彩虹扫过（正文除外）**（会话记录里**最新的一条**——命令卡或 `data-variant="think"` 思考行，`setLatest` 取最晚并标 `data-rf-latest`——其 `_title`/`_summary`/`_fileLink` 被 `ToolAccent.css` 的**流动彩虹渐变**（`background-clip: text`）扫过每个字符，输出正文不参与；**跟随最新行动并持续**，直到**下一条命令/思考出现**或**其后方出现正文回复**（助手纯文本 markdown）才熄灭（read/edit 这类瞬间完成的命令不会提前熄灭）——纯 CSS、不重渲染 DOM、`prefers-reduced-motion` 冻结为静态彩虹；**受 `commandSweep` 开关控制**（本模块把设置写成 `<html>` 的 `data-rf-sweep='on'|'off'` 门控；`setLatest` **每次从 DOM 确定性重算**最新一条 `[data-tool]`/`[data-variant="think"]` 行，并检查其后方是否紧接正文文本块来判定「已熄灭」，无 sticky 状态——扫字选择器只用并集标出的 `data-rf-latest`，故**独立于 `commandColor` 上色开关**）。命令上色本身**受 `commandColor` 开关控制**（关掉时本模块停止打标签并清除已有 `data-rf-tool-cat`/`title`，命令卡恢复原厂外观）。
