...
- **可配置**：⚙ 设置面板自身也随语言切换（跟随挂件的语言开关），实时调节语言、
  数字格式/字号、标签、连击、粒子、暴击阈值/比例、音效、边缘泛光、霓虹闪烁/
  乱码故障强度（关/低/中/高）；全部设置（含位置、缩放与面板各选项）写入
  `localStorage`。面板里的 ⚡「测试特效」按钮可随时重播完整暴击动效（伤害数字、
  粒子、连击、边缘泛光、音效），不会改变真实计数。
- **支持减弱动效**：当系统开启 `prefers-reduced-motion` 时，装饰性动效——数字
  弹跳、暴击抖动/闪光、边缘泛光、霓虹闪烁、乱码与常驻粒子漂移——都会关闭，而核心
  暴击反馈（浮动伤害数字 + 粒子迸溅）仍保留。
