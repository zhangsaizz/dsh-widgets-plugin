...
export class BalanceController implements HostObservable<BalanceViewState> {
   ...
   * @param refreshIntervalMs - validated periodic refresh interval.
   */
  constructor(
  ...
  private async reconcile(): Promise<void> {
    ...
    const selected = await this.resolveSelection(sessionId)
    ...
    const carriedPromise = typeof this.remote.query === 'function'
      ? this.remote.query({ provider: provider ?? '', ...(model === null ? {} : { model }) }).catch(() => null)
