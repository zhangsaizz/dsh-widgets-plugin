...
- **可配置**：小组件管理页把彩虹流光当成普通挂件——**启用/停用**开关与工具栏
  ...
  **整体透明度**（40/70/100%）、**速度灵敏度**（0.5×/1×/1.5×）、**思考冷色调
  开关**、**最新行动彩虹扫字开关**，外加**命令文字颜色**——其区标题行右侧是
  **「命令按类别上色」总开关**（关掉时色板灰化待命，仅保存颜色、不上色）；下方为
  **每类命令**（shell、read、search、write、edit、code、web、ask、plan、memory、think、other）
  各配一个 **web 命令卡同款工具名与官方图标**（如 bash、read_file、apply_patch、
  run_code、web_fetch、ask_user_question、remember），名字与图标随该行取色**实时同色**，
  每行一个取色器 + 单行恢复默认——持久化到 `localStorage`
...
- **会话命令卡按类型上色**：会话记录（transcript）里模型的每个**工具调用
  ...
  未知工具归入"命令"默认色。**颜色可在配置面板按类别自定义**（见「可配置」，写入 `--rf-tool-*` 变量实时生效）。**最新行动（命令/思考）被彩虹扫过（正文除外）**：会话记录里**最新的一条**——命令卡或 **「Think」思考行**——其标题/摘要/文件名被一道**流动的彩虹渐变**（`background-clip: text`）扫过每个字符，其**输出正文不参与**；它**跟随最新行动持续**（read/edit 这类瞬间完成的命令也能亮起，不依赖 `running`），直到**其后出现正文回复**（助手纯文本，用 flow-item 结构精确判定，不会误把 composer 当正文）才熄灭。每次从 DOM 确定性重算、无 sticky 状态，纯 CSS 实现、不重渲染 DOM，`prefers-reduced-motion` 下冻结为静态彩虹；可在配置面板「最新行动彩虹扫字开关」关闭。命令上色本身**可由「命令按类别上色开关」控制**（默认开，关掉即恢复命令卡原厂外观），且该上色与扫字均独立于输入框光晕开关。
